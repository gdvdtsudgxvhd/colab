...
def launch_direct_ssh(username, ip_address, port,
    ...
    run_command(
        f'scp -o "StrictHostKeyChecking=no" -P {port} {username}@{ip_address}:{public_key_path} ~/.ssh/authorized_keys')
    ...
    run_command(
        f'autossh -o "StrictHostKeyChecking=no" -f -T -N {reverse_tunnel_ports} {username}@{ip_address} -p {port}')
